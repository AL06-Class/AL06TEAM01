...
export function ReservationPaymentCard({ selectedProvider, careRequest, status, onPay }: ReservationPaymentCardProps) {
  ...
  const paymentPlanLabel = careRequest.paymentPlan === "subscription" ? "정기 패키지" : "건별 결제";

  return (
    <section className="section-card compact-card" aria-labelledby="reservation-payment-title">
      <div className="section-card-header">
        <p className="section-kicker">핵심 기능 2 - 예약 / 결제</p>
      ...
      <dl className="info-list">
        ...
        <div>
          <dt>도움 유형</dt>
          <dd>{careRequest.careType || "장보기 동행"}</dd>
        </div>
        <div>
          <dt>결제 방식</dt>
        ...
        <div>
          <dt>선택한 가치이웃</dt>
          <dd>{selectedProvider?.name ?? "가치이웃을 선택해주세요"}</dd>
        </div>
        <div>
          <dt>신청자 제안 금액</dt>
